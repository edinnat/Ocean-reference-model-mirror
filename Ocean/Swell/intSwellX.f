...
c    Var   : variance des pentes en x (direction du vent)
c-----------------------------------------------------------------------

       subroutine intSwellX (sigma, KMax, N, Swell, Var)
       ...
       do ikx = 1, N(1)
          ...
          do iky = 1, N(2)
             ky = kymin + (iky-1)*dky
             Var = Var + Swell(ikx,iky)*kx**2*dkx*dky !projection du spectre des hauteur sur kx
             if ((kx**2+ky**2).gt.10) print*, '!!!' ! si k>kd probleme 
          enddo
       enddo
